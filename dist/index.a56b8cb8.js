...
function updateNPCPosition() {
    ...
    if (npcPosX > window.innerWidth - npc.offsetWidth) npcPosX = window.innerWidth - npc.offsetWidth;
...
function updateNPCBehavior() {
    ...
    } else // Le joueur est loin, le NPC reprend son comportement normal
...
function checkCollision() {
    ...
    if (playerRect.right - tolerance > gameBoardRect.left && playerRect.left + tolerance < gameBoardRect.right && playerRect.bottom - tolerance > gameBoardRect.top && playerRect.top + tolerance < gameBoardRect.bottom) handlePlayerCollision(gameBoardRect);
    else if (playerRect.right - tolerance > towerRect.left && playerRect.left + tolerance < towerRect.right && playerRect.bottom - tolerance > towerRect.top && playerRect.top + tolerance < towerRect.bottom) handlePlayerCollision(towerRect);
    // Gestion des collisions pour le NPC
    if (npcRect.right - tolerance > gameBoardRect.left && npcRect.left + tolerance < gameBoardRect.right && npcRect.bottom - tolerance > gameBoardRect.top && npcRect.top + tolerance < gameBoardRect.bottom) handleNPCCollision(gameBoardRect);
    else if (npcRect.right - tolerance > towerRect.left && npcRect.left + tolerance < towerRect.right && npcRect.bottom - tolerance > towerRect.top && npcRect.top + tolerance < towerRect.bottom) handleNPCCollision(towerRect);
}
function handlePlayerCollision(collisionRect) {
    ...
    if (verticalCollision < horizontalCollisionLeft && verticalCollision < horizontalCollisionRight) {
        // Collision verticale
        velocity = 0;
        ...
        // Collision latérale
        if (facingDirection === "right") playerPosX = collisionRect.left - player.offsetWidth;
        else if (facingDirection === "left") playerPosX = collisionRect.right;
    }
    player.style.top = `${playerPosY}px`;
...
function handleNPCCollision(collisionRect) {
    ...
    if (verticalCollision < horizontalCollisionLeft && verticalCollision < horizontalCollisionRight) {
        ...
        // Collision latérale
        if (npcFacingDirection === "right") npcPosX = collisionRect.left - npc.offsetWidth;
        else if (npcFacingDirection === "left") npcPosX = collisionRect.right;
    }
    npc.style.top = `${npcPosY}px`;
    npc.style.left = `${npcPosX}px`;
}
...
function updatePlayerPosition() {
    ...
    if (movingRight) {
        ...
        if (playerPosX > window.innerWidth - player.offsetWidth) playerPosX = window.innerWidth - player.offsetWidth;
    }
    player.style.left = `${playerPosX}px`;
}
...
document.addEventListener("keydown", function(event) {
    ...
    else if (event.key === "ArrowRight" || event.key === "d") movingRight = true;
});
document.addEventListener("keyup", function(event) {
    if (event.key === "ArrowLeft" || event.key === "a") movingLeft = false;
    else if (event.key === "ArrowRight" || event.key === "d") movingRight = false;
});
...
//# sourceMappingURL=index.a56b8cb8.js.map
